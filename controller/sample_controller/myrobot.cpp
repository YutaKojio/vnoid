...
void MyRobot::Init(SimpleControllerIO* io){
    ...
    param.base_to_hip     [0] = Vector3(0.0, -0.09, -0.1);
    param.base_to_hip     [1] = Vector3(0.0,  0.09, -0.1);
    ...
    param.arm_joint_index [0] =  6;
    param.arm_joint_index [1] = 19;
    param.leg_joint_index [0] = 0;
    param.leg_joint_index [1] = 13;
    param.upper_arm_length = 0.2;
    param.lower_arm_length = 0.2;
    param.upper_leg_length = 0.3;
    param.lower_leg_length = 0.4;
    ik_solver.foot_off = Vector3(-0.07, 0, 0);
    ...
    footstep.steps.push_back(Step(0.0, 0.0, 0.18, 0.0, 0.0, 0.5, 1));
    // foot placement of the initial step must be specified
    footstep.steps[0].foot_pos[0] = Vector3(0.0, -0.18/2.0, 0.0);
    footstep.steps[0].foot_pos[1] = Vector3(0.0,  0.18/2.0, 0.0);
    footstep_planner.Plan(param, footstep);

    // init stepping controller
    stepping_controller.swing_height = 0.10;
    stepping_controller.dsp_duration = 0.18;
    ...
    stabilizer.force_ctrl_gain          = 0.00001;
    stabilizer.force_ctrl_limit         = 0.05;
    stabilizer.moment_ctrl_damping      = 5.0;
    stabilizer.moment_ctrl_gain         = 0.00001;
    stabilizer.moment_ctrl_limit        = 3.0;
...
void MyRobot::Control(){
	...
	if(timer.count % 10 == 0){
		...
		*/
		...
		Step step;
		step.stride   = 0.15; //-max_stride*joystick.getPosition(Joystick::L_STICK_V_AXIS);
    ...
    centroid.com_pos_ref = Vector3(0.0, 0.0, 1.5);
    foot[0].pos_ref      = Vector3(-0.07, -0.09, 0.8);
    foot[1].pos_ref      = Vector3(-0.07,  0.09, 0.8);
    */
